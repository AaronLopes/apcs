...
public final class StdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    ***************************************************************************/
     ...
     *
     * @param  xPoly an array of all the <em>x</em>-coordinates of the polygon
     * @param  yPoly an array of all the <em>y</em>-coordinates of the polygon
     * @throws IllegalArgumentException unless {@code x[]} and {@code y[]}
     *         are of the same length
     */
    public static void polygon(double[] xPoly, double[] yPoly) {
        if (xPoly == null) throw new NullPointerException();
        if (yPoly == null) throw new NullPointerException();
        int n1 = xPoly.length;
        int n2 = yPoly.length;
        if (n1 != n2) throw new IllegalArgumentException("arrays must be of the same length");
        int n = n1;
        GeneralPath path = new GeneralPath();
        path.moveTo((float) scaleX(xPoly[0]), (float) scaleY(yPoly[0]));
        for (int i = 0; i < n; i++)
            path.lineTo((float) scaleX(xPoly[i]), (float) scaleY(yPoly[i]));
        path.closePath();
        offscreen.draw(path);
        draw();
    }
